...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs: InputsType) -> InputsType:
        ...
        videos = [inputs[i]['videos'][0] for i in range(len(inputs))]
        trajectory_infos = None
        if self.use_gym_env:
            trajectory_infos = [inputs[i]['trajectory_info'] for i in range(len(inputs))]
        self._log_metrics(batch_encoded_inputs, messages, completions, total_rewards, total_rewards_per_func, videos,
                          trajectory_infos)
    ...
    def _log_metrics(self, inputs, messages, completions, rewards, rewards_per_func, videos, trajectory_infos=None):
        ...
        self._logs['videos'].extend(gather_object(videos))
